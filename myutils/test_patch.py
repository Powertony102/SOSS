...
def var_all_case(model, num_classes, patch_size=(112, 112, 80), stride_xy=18, stride_z=4, dataset_name="LA", dataset_path=None):
    if dataset_name == "LA":
        assert dataset_path is not None, "dataset_path must be provided for LA dataset!"
        with open(f'{dataset_path}/train.list', 'r') as f:
            image_list = f.readlines()
        image_list = [f"{dataset_path}/2018LA_Seg_Training Set/" + item.replace('\n', '') + "/mri_norm2.h5" for item in image_list]
