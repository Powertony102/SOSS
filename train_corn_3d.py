...
parser.add_argument('--exp', type=str, default='corn', help='exp_name')
...
parser.add_argument('--temperature', type=float, default=0.4, help='temperature of sharpening')
...
if __name__ == "__main__":
    ...
    # print(f"Training dataset size: {len(db_train)}")

    # Print the image size of the training dataset  
    # print(f"Training dataset image size: {db_train[0]['image'].shape}")
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            # print(f"labeled_features_v shape:{labeled_features_v.shape}")
            ...
            # print(f"b:{b}, h:{h}, w:{w}, d:{d}, labeled_features_dim:{labeled_features_dim}")
            ...
            # print(f"labeled_features_v after view shape:{labeled_features_v.view(-1, labeled_features_dim).shape}")
            ...
            dynamic_feature_pool.update_labeled_features(labeled_features_correct_list, labels_correct_list,
                                                labeled_index_list)

            # sample memory bank size labeled features from memory bank
            memory = dynamic_feature_pool.sample_labeled_features(args.memory_num)
            ...
            # print(f"mask_consist_unlabeled shape:{mask_consist_unlabeled.shape}")
            ...
            # print(f"{corr_v_available}")
            if corr_v_available and corr_a_available:
                num_samples = corr_a.shape[0]
                cross_entropy_loss = F.cross_entropy(corr_v, corr_a)
                loss_d = cross_entropy_loss / num_samples
            else:
                loss_d = 0
            ...
            if iter_num >= 1000 and iter_num % 500 == 0:
                ...
                if args.dataset_name == "LA":
                    dice_sample = test_patch.var_all_case(model, num_classes=num_classes, patch_size=patch_size,
                                                          stride_xy=18, stride_z=4, dataset_name='LA', dataset_path=args.dataset_path)
                # Notification!
